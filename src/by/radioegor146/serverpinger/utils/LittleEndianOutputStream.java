...
public class LittleEndianOutputStream extends FilterOutputStream {
     ...
     * at <code>offset</code> to the underlying output stream.
     *
     * @param data the data.
     ...
     * byte. If the argument is true, the byte value 1 is written. If the
     * argument is false, the byte value <code>0</code> in written.
     *
    ...
    /**
     * Writes a two byte <code>char</code> to the underlying output stream in
     * little endian order, low byte first.
     *
    ...
    /**
     * Writes a four-byte <code>int</code> to the underlying output stream in
    ...
    /**
     * Writes a 4 byte Java float to the underlying output stream in little
     * endian order.
     *
    ...
    /**
     * Writes an 8 byte Java double to the underlying output stream in little
     * endian order.
     *
     ...
     * output stream using UTF-8 encoding. This method first writes a two byte
     ...
     * string, not the number of characters in the string. Next each character
     * of the string is written using the UTF-8 encoding for the character.
     *
     ...
     * possible that this number is temporarily less than the actual number of
     * bytes written.)
     *
